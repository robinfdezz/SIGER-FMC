...
export const InlineConfirmButton = ({
  ...
  const baseClasses = isCard
    ...
    : isPrimary
    ? 'h-[38px] px-3.5 rounded-xl bg-red-600 hover:bg-red-700 active:bg-red-800 text-white text-xs font-semibold shadow-xs shrink-0 transition-all duration-200 select-none'
    : 'px-3 py-1.5 rounded-xl text-xs font-medium border border-neutral-200 dark:border-neutral-800 transition-all duration-200 select-none';
  ...
  if (!isConfirming) {
    return (
      <button
        ...
        className={`${baseClasses} hover:bg-neutral-100 dark:hover:bg-neutral-800/80 flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
      >
        <div className="flex items-center justify-center gap-1.5 transition-all duration-200 ease-out opacity-100 scale-100">
          {loading ? (
            <Loader2 size={isCard ? 11 : 14} className="animate-spin shrink-0" />
          ) : Icon ? (
            <Icon size={isCard ? 11 : 14} className={isCard ? 'text-red-500 shrink-0' : 'text-white shrink-0'} />
          ) : null}
          <span>{text}</span>
        </div>
      </button>
  ...
  return (
    <div
      ...
      className={`${baseClasses} flex items-center justify-between gap-1.5 ${confirmClassName || className}`}
    >
      ...
      <div
        className={`flex items-center gap-0.5 sm:gap-1 shrink-0 transition-all duration-200 ease-out ${
          isExiting ? 'opacity-0 translate-x-1.5 scale-95' : 'opacity-100 translate-x-0 scale-100'
        }`}
      >
        ...
        <button
          ...
          title="Confirmar"
          className={`rounded-md p-1 flex items-center justify-center transition-all duration-150 active:scale-90 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
            isCard
        ...
        >
          {loading ? (
            <Loader2 size={isCard ? 13 : 15} className="animate-spin text-current" />
          ) : (
            <Check size={isCard ? 14 : 16} strokeWidth={2.4} className="shrink-0" />
          )}
        </button>
        ...
        <button
          ...
          title="Cancelar"
          className={`rounded-md p-1 flex items-center justify-center transition-all duration-150 active:scale-90 cursor-pointer ${
            isCard
        ...
        >
          <X size={isCard ? 14 : 16} strokeWidth={2.4} className="shrink-0" />
        </button>
      </div>
    </div>
  );
